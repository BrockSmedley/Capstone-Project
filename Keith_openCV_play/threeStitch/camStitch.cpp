...
int streaming(VideoCapture cap1, VideoCapture cap2, VideoCapture cap3, double start)
{
    Mat fr1, fr2, fr3, copy1, copy2, copy3, pano;
...
int main(int argc, char *argv[])
{
	...
	VideoCapture cap3("nvcamerasrc sensor-id=1 ! video/x-raw(memory:NVMM), width=(int)640, height=(int)480, format=(string)I420, framerate=(fraction)30/1 ! nvvidconv flip-method=0 ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink", CAP_GSTREAMER); //output closest to center of board
	...
	while(true)
	{
		...
		time(&start);
		test = streaming(cap1, cap2, cap3, start);
