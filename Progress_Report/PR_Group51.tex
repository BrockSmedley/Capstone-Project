...
\subsection{Overview}
This document will provide briefly recaps and summaries weekly of our project in the fall 
term 2017. The goal of the project is aimed to build the multi-camera, SoM based, real-time
video processing for UAS and VR/AR application and cut down the cost, and the primary job 
we processed in the term is researching on adequate hardware and software pieces, which are 
asked to meet the requirements and be able to be connected to each other. Based on hardware 
pieces, the process of researching on software started in last weeks of the term. The 
possible softwares are included in the NVIDIA Jetpack or open source, so these does not 
influence the cost.
