...
low due to the information from Ridge Run that requires the use of 
...
In our pursuit to produce multi-camera output two setups have been attempted. Our first 
...
the Auvidea J20-2 module. Then multiple wikis by Ridge Run were followed to setup and configure 
the kernel for this hardware combination. While building the kernel in the TX2 instead of 
out host laptop we experienced \textt{WARNING: root device  does not exist} multiple times
...
supporting the module, but was still unable to 
successfully produce a multi-camera input even after obtaining patches from Ridge Run. \\
